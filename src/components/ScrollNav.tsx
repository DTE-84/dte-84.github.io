    ...
    <div className="scroll-nav-container fixed top-1/2 left-[25px] -translate-y-1/2 w-3 h-[400px] z-[1000] hidden xl:block">
      ...
      {sections.map((section) => (
        ...
        >
          ...
          <span className={`orb-label ml-[55px] text-[14px] font-bold text-accent tracking-[2px] whitespace-nowrap transition-all duration-400 group-hover:opacity-100 group-hover:translate-x-0 ${activeSection === section.id ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-[15px]'}`}>
            {section.label}
          </span>
          {/* Tactical connecting line */}
          <div className={`absolute left-[15px] h-[1px] bg-gradient-to-r from-accent to-transparent opacity-40 transition-all duration-400 pointer-events-none group-hover:w-10 ${activeSection === section.id ? 'w-10' : 'w-0'}`} />
        </div>
      ))}
    </div>
  );
