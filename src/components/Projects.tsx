  ...
  const projects = [
    {
      ...
      year: '2026',
      description: 'High-fidelity clinical drafting tool for Missouri DMH providers, automating complex ISP documentation with zero-knowledge architecture.',
    ...
    },
    {
      ...
      category: 'Fintech',
      year: '2026',
      description: 'Real-time cryptocurrency tracking with live price feeds and dynamic visualization.',
    ...
    },
    {
      ...
      category: 'Creative',
      year: '2026',
      description: 'A professional high-fidelity modeling portfolio featuring immersive visual galleries.',
    ...
    },
    {
        ...
        category: 'Service',
        year: '2026',
        description: 'Premium landing page for a St. Louis landscaping company, featuring a custom interactive "Before & After" engine.',
    ...
    },
    {
      ...
      category: 'Saas',
      year: '2025',
      description: 'An automotive marketplace optimized for high-performance discovery flows.',
    ...
    },
    {
      ...
      category: 'Business',
      year: '2025',
      description: 'High-converting business landing page emphasizing performance and premium transitions.',
      tags: ['HTML5', 'CSS3', 'JS'],
    ...
    },
    {
      ...
      category: 'Commerce',
      year: '2025',
      description: 'Modern e-commerce solution exploring advanced state management and React Hooks.',
    ...
    },
    {
      ...
      category: 'Archive',
      year: '2025',
      description: 'My first dive into software development. A retrospective on my learning journey.',
      tags: ['HTML5', 'CSS3', 'JS'],
      ...
      <div className="container-main">
        <div className="flex items-center gap-6 mb-32">
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-widest uppercase">
            <span style={{ color: 'var(--theme-offset)', opacity: 0.8 }}>02 //</span> Exploratory Case Study
          </h2>
        ...
        {projects.filter(p => p.featured).map(project => (
          <div key={project.id} className="group relative mb-48">
            <div className="mb-0.5 flex flex-col pl-4">
              ...
              <span className="text-[10px] font-mono text-accent/40 tracking-[0.2em] ml-4">REF_ID: {project.id}</span>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-16 items-center">
              ...
              <div className="relative border-l-4 border-accent pl-10 py-8 bg-accent/[0.03] backdrop-blur-sm rounded-r-2xl group-hover:bg-accent/[0.05] transition-all duration-500">
                <div className="flex items-center gap-4 mb-6">
                  <span className="px-3 py-1.5 category-badge rounded-none text-[10px] font-bold border border-accent/20 text-accent bg-accent/5 tracking-widest uppercase">{project.category}</span>
                  <span className="text-xs text-zinc-400 font-mono tracking-[0.2em]">{project.year}</span>
                </div>
                <h3 className="text-4xl font-bold text-white mb-6 group-hover:text-accent transition-colors uppercase tracking-widest leading-tight">{project.title}</h3>
                <p className="text-white/70 text-lg leading-relaxed mb-8 font-medium">{project.description}</p>
                <div className="flex flex-wrap gap-3 mb-10 text-accent font-bold">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] bg-accent/10 px-3 py-1.5 rounded uppercase tracking-widest border border-accent/20">{tag}</span>
                  ))}
                </div>
                <div className="flex gap-8">
                  <a href={project.link} target="_blank" className="inline-flex items-center text-sm font-bold text-accent border-b-2 border-accent pb-1 hover:border-white hover:text-white transition-all uppercase tracking-widest">
        ...
        <div className="flex items-center gap-6 mb-32">
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-widest uppercase">
            <span style={{ color: 'var(--theme-offset)', opacity: 0.8 }}>03 //</span> The Archives
          </h2>
          <div className="h-[1px] flex-grow bg-accent/60"></div>
        </div>
          ...
          {projects.filter(p => !p.featured).map((project, idx) => (
            <div key={project.id} className="group grid md:grid-cols-2 gap-16 items-center">
              ...
              <div className={idx % 2 !== 0 ? 'md:order-1' : ''}>
                ...
                <h3 className="text-3xl font-bold text-white mb-4 group-hover:text-accent transition-colors uppercase tracking-widest">{project.title}</h3>
                <p className="text-white/70 text-lg leading-relaxed mb-8 font-medium">{project.description}</p>
                <div className="flex flex-wrap gap-3 mb-10 text-accent font-bold">
                  {project.tags.map(tag => (
                    <span key={tag} className="px-3 py-1.5 tech-tag-chip rounded-none text-[10px] font-bold border border-accent/20 text-accent bg-accent/5 tracking-widest uppercase">{tag}</span>
                  ))}
                </div>
                <a href={project.link} target="_blank" className="inline-flex items-center text-sm font-bold text-accent border-b-2 border-accent pb-1 hover:border-white hover:text-white transition-all uppercase tracking-widest">
                  Uplink Project <Icon icon="solar:arrow-right-up-linear" className="ml-2" />
                </a>
              </div>
            </div>
          ))}
